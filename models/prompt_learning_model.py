'''
Learn prompt token (concept token), prompt tuning for given dataset
tasks are
1. Prompt tune model for given code dataset
2. Save model input embedding with predefined set of tokens 
(reference github https://github.com/WANGXinyiLinda/concept-based-demonstration-selection/tree/main)
'''
...
class PromptLearningModel(object):
    def __init__(self, gpt2="gpt2-large", logger=None, 
        out_dir=None, fp16=False, local_rank=-1, soft_prefix=True, n_tokens=10, 
    ...
    def do_train(self, data, batch_size, num_training_steps, save_period, log_period,
        ...
        train_dataloader , val_dataloader = data.get_dataloader(batch_size)
        ...
        for epoch in range(num_training_steps):
            ...
            for batch in tqdm(train_dataloader):
                ...
                if global_step % log_period == 0 or global_step == 1:
                    self.logger.info("local rank %d\tglobal step %d\ttrain loss %.2f" % (self.local_rank, global_step, np.mean(train_losses)))
                    ##train_losses = []    we are calculating running average of train loss

                    ##run on validation set
                    self.do_validation(val_dataloader)
    ...
    def do_validation(self, val_dataloader):
        ...
        self.logger.info("local rank %d\tval loss %.2f" % (self.local_rank,  np.mean(val_losses)))
