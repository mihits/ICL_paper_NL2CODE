...
from utils.load_prompt_learning_dataset import PromptLearningDataset
from models.prompt_learning_model import PromptLearningModel

from utils.load_prompt_learning_dataset import load_data
...
def main(logger, args):
    ...
    if args.task is not None:
        train_data = load_data(args.task, "train", args.k, seed=args.seed, 
                config_split=args.split, data_dir=args.data_dir)    #args.data_dir = 'data/'
    ...
    metaicl_data = PromptLearningDataset(logger, args.gpt2, args.method, args.use_demonstrations,
                               args.test_k,max_length= max_length,max_length_per_example= max_length_per_example,
    ...
    ###### tensorize data (store file in "tensorized" dir) 
    if args.do_tensorize:
        print("\nStarted Tensorization\n")
        metaicl_data.tensorize_for_training(train_data, keyword=keyword, 
        seed=args.seed, use_random_english_words=False)
        print("\nFinished Tensorization\n")
    

    ######### load tensorize data
    metaicl_data.load_tensorized_data(train_data,keyword=keyword,seed = args.seed)
    ...
    metaicl_model = PromptLearningModel(args.gpt2, logger, 
    ...
    ##metaicl_model.train()
    metaicl_model.do_train(metaicl_data, args.batch_size, num_training_steps, args.save_period, args.log_period)
...
if __name__=='__main__':
    ...
    parser.add_argument("--do_sanity_check", default=False, action="store_true")
    parser.add_argument("--save_period", type=int, default=1000)
    parser.add_argument("--log_period", type=int, default=250)
